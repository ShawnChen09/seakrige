...
class SeaKrige(SeaPath):
    def __init__(self, known_points, z, shapefile, resolution = 0.1,):
        super().__init__(shapefile, resolution)
    ...
    def _calc_distmx(self):
        dist_mx = [[max(self.dist_func(apoint, bpoint), distance.euclidean(apoint, bpoint)) for bpoint in self.known_points]
    ...
    def _calc_semivariance(self):
        ...
        z = (z - z.mean())/(z.std())
        n = len(z)
        self.semivarmx = np.array([[0.5 * (z[i] - z[j])**2 for j in range(n)]
                                                           for i in range(n)])
    ...
    def _fit_variogram(self):
        ...
        for model in self.models:
            ...
            _, _, r2 = fit_model.fit_variogram(self.bin_centers, self.gamma, return_r2=True)
    ...
    def plot_fit_variogram(self):
        ...
        ax.scatter(self.distmx.flatten(), self.semivarmx.flatten(), color='blue', label='Experimental Variogram')
    ...
    def _get_variogram_matrix(self):
        ...
        self.variogram_matrix = np.array([[self.fit_model.variogram(self.distmx[i, j]) for j in range(n)] 
                                                                                 for i in range(n)])

    def _predict_kriging(self, unknown_point):
        n = self.known_points.shape[0]
        distances = np.array([self.dist_func(self.known_points[i], unknown_point) for i in range(n)])
    ...
    def predict(self):
        ...
        for query_point in zip(self.X.flatten(), self.Y.flatten()):
            try:
                predict_value = self._predict_kriging(query_point,)
            except:
                predict_value = 0
            self.Z.append(predict_value)
        self.Z = np.array(self.Z).reshape(self.X.shape)
        self.Z[self.grid_mask] = None

    def plot_predict_result(self, show_base_map = True):
        fig, ax = plt.subplots()
        self.gdf.plot(facecolor='none', edgecolor='black', linewidth=1.5, zorder=5, ax=ax)
        cx.add_basemap(ax, crs=self.crs)
        
        Z_reverse = self.Z[::-1]
        plt.imshow(Z_reverse, extent=[self.X.min(), self.X.max(), self.Y.min(), self.Y.max()], interpolation="gaussian")
